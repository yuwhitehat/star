...
public class StreamDetailTest {
    ...
    public void filter() {
        ...
        orders.stream()
                .filter(order -> Objects.nonNull(order) && order.getPlacedAt().isAfter(LocalDateTime.now().minusMonths(6)) && order.getTotalPrice() > 40)
                .forEach(System.out::println);
    }
    ...
    public void map() {
        ...
        Long result = orders.stream()
                .mapToLong(order -> order.getOrderItemList().stream().mapToLong(OrderItem::getProductQuantity).sum())
                .sum();
